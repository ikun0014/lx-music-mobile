...
export const cheatTip = async() => {
  ...
  return tipDialog({
    title: '禁止传播提升',
    message: `1. 本项目无微信公众号之类的官方账号，也未在小米、华为、vivo等应用商店发布应用，商店内的“LX Music Mod”、“洛雪音乐助手”相关的应用全部属于假冒应用，谨防被骗。
2. 本软件完全无广告且无引流（如需要加群、关注公众号之类才能使用或者升级）的行为，若你使用过程中遇到广告或者引流的信息，则表明你当前运行的软件是第三方修改版。
3. 目前本项目的原始发布地址只有 GitHub 及 蓝奏网盘 （在设置-关于有说明），其他渠道均为第三方转载发布，可信度请自行鉴别。(此处点名奇妙应用，你们那些什么纠察官部分有点离谱了)`,
    btnText: '我知道了 (Close)',
    bgClose: false,
  }).then(() => {
